...
if (
  ...
  (F.operate = {
    ...
    sin: function (t) {
      return (
        Math.min(1, Math.floor(F.amod((2 * t) / Math.PI - 1, 4) / 2)) *
          (Math.floor(F.amod((2 * t) / Math.PI, 2)) *
            (Math.floor((2 * t) / Math.PI) - (2 * t) / Math.PI) ** 2 +
            Math.floor(F.amod((2 * t) / Math.PI + 1, 2)) *
              (2 - (Math.floor((-2 * t) / Math.PI) + (2 * t) / Math.PI) ** 2)) +
        Math.min(1, Math.floor(F.amod((2 * t) / Math.PI + 1, 4) / 2)) *
          (Math.floor(F.amod((-2 * t) / Math.PI, 2)) *
            (Math.floor((-2 * t) / Math.PI) + (2 * t) / Math.PI) ** 2 +
            Math.floor(F.amod((-2 * t) / Math.PI + 1, 2)) *
  ...
  for (var i in mouseEvents) window[mouseEvents[i]] = F.setMouse;
