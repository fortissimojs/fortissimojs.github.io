...
const F = {
    ...
    deprecateWarning: function (oldName, newName) {
        if (F.deprecateWarned[oldName]) {
            return;
        }
        F.deprecateWarned[oldName] = true;
    ...
    border: function (number, min, max) {
        F.deprecateWarning("border", "clamp");
        return F.clamp(number, min, max);
    },
    ...
    collide: {
        ...
        rect2circle: function (a, b) {
            return F.collide.circle2rect(b, a);
        },
